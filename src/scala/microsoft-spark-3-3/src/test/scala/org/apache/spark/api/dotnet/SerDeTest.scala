...
class SerDeTest {
  ...
  @Test
  def shouldReadArrayOfDoubles(): Unit = {
    val input = givenInput(in => {
      in.writeByte('l') // type list
    ...
    assertArrayEquals(Array[Double](1d, 2d), serDe.readObject(input).asInstanceOf[Array[Double]], 0.0001d)
  }

  @Test
  def shouldReadArrayOfFloats(): Unit = {
    val input = givenInput(in => {
      in.writeByte('l') // type list
      in.writeByte('f') // type descriptor
      in.writeInt(2) // number of elements
      in.writeFloat(1f)
      in.writeFloat(2f)
    })

    assertArrayEquals(Array[Float](1f, 2f), serDe.readObject(input).asInstanceOf[Array[Float]], 0.0001f)
  }  
  ...
  def shouldWritePrimitiveTypes(): Unit = {
    val in = whenOutput(out => {
      ...
      serDe.writeObject(out, 42.24d.asInstanceOf[Object])
    ...
    assertEquals(in.readDouble(), 42.24d, 0.000001)
  ...
  @Test
  def shouldWriteArrayOfDouble(): Unit = {
    val payload = Array(1d, 2d, 3d, 4d)
    val in = whenOutput(out => {
      serDe.writeObject(out, payload)
    })

    assertEquals(in.readByte(), 'l') // array type
    assertEquals(in.readByte(), 'd') // type of element in array
    assertEquals(in.readInt(), 4) // array length
    assertEquals(in.readDouble(), 1d, 0.000001)
    assertEquals(in.readDouble(), 2d, 0.000001)
    assertEquals(in.readDouble(), 3d, 0.000001)
    assertEquals(in.readDouble(), 4d, 0.000001)
  }

  @Test
  def shouldWriteArrayOfFloat(): Unit = {
    val payload = Array(1f, 2f, 3f, 4f)
    val in = whenOutput(out => {
      serDe.writeObject(out, payload)
    })

    assertEquals(in.readByte(), 'l') // array type
    assertEquals(in.readByte(), 'f') // type of element in array
    assertEquals(in.readInt(), 4) // array length
    assertEquals(in.readFloat(), 1f, 0.000001)
    assertEquals(in.readFloat(), 2f, 0.000001)
    assertEquals(in.readFloat(), 3f, 0.000001)
    assertEquals(in.readFloat(), 4f, 0.000001)
  }  
